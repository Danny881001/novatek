...
#define MMC_STATUS_TRANS_ST (4<<9)
...
#define EXT_CSD_PART_ATTR_ENH_USER 	(1 << 0)
...
struct mmc {
	...
	u8 en_egroup;// for emmc erase
#endif
#if defined(CONFIG_EMMC_WRITE_RELIABILITY_TEST)
	u8 en_wr;// for write_reliability 
	u32		wr_test_cnt;	/* wr test count	*/
	u32		wr_total_test_cnt;	/* wr total test count	*/
#endif
#if defined(CONFIG_NVT_MMC_CMD_LOG)
struct mmc_cmd log_cmd_dump[20];
u8  last_log_idx;
#endif
};
...
#if defined(CONFIG_EMMC_WRITE_RELIABILITY_TEST)
//emmc write reliability test
#define WR_PAT_SIZE (0x100000) //1MB
#define WR_HEADER_ADDR (0x204800) //100MB
typedef struct wr_header {
	__be32		wr_test_cnt;	/* wr_test count	*/
	__be32		wr_total_test_cnt;	/* wr_total test count	*/
	__be32		wr_dcrc;	/* data crc checksum	*/
	__be32		wr_pcrc;	/* pattern crc checksum	*/
	__be32		wr_start;	/* start sector	*/
	__be32		wr_size;	/* bytes	*/
	__be32		wr_test_pat_start;	/* start sector	*/
	__be32		wr_test_pat_size;	/* bytes	*/
	__be32		wr_header_start;	/* test result sector	*/
} wr_header_t;
int nvt_mmc_power_down(u32 wr_test_cnt, u32 wr_total_cnt);
