...
HAVE_VENDOR_3RD_PARTY_LIB = $(if $(wildcard $(srctree)/board/$(VENDOR)/3rd_party/Makefile),y,n)

libs-y += lib/
libs-$(HAVE_VENDOR_COMMON_LIB) += board/$(VENDOR)/common/
libs-$(HAVE_VENDOR_3RD_PARTY_LIB) += board/$(VENDOR)/3rd_party/
...
ifndef CONFIG_NA51000
libs-y += drivers/mtd/spi/
else
libs-y += drivers/mtd/nvt_flash_spi/
libs-y += drivers/mtd/spiflash/
endif
libs-y += drivers/net/
...
RSA_PUB_KEY_FILE = ./board/novatek/$(CONFIG_SIGN_TOOL_DIR)/rsa_priv.pem
ifeq ($(CONFIG_NVT_SECURE_BOOT),y)
	SIGN_UBOOT = $(shell pwd)/nvt_sign_bin $(RSA_PUB_KEY_FILE) u-boot.bin
	SIGN_UBOOT_IMG=
endif

ifeq ($(CONFIG_NVT_SECURE_SIGN_COMMON_FORMAT),y)
	SIGN_UBOOT_IMG = $(shell pwd)/nvt_sign_comm_img $(RSA_PUB_KEY_FILE) u-boot.img
	SIGN_UBOOT=
endif
...
u-boot-dtb.img u-boot.img u-boot.kwb u-boot.pbl: \
	...
	@cp u-boot.img u-boot.img.unsign
	@$(SIGN_UBOOT_IMG)
