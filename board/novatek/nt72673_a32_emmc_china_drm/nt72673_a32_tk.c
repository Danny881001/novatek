...
#define NVT_EBURNER_V2_ADC_VAL                  0xd1
#define NVT_EBURNER_V2_ADC_RANGE                0x6
...
void nt72673_a32_emmc_burn_image(void)
{
	int ret;
	u32 reg_val, tmp_val;
	char cmd[128] = {0};
	...
	ret = run_command("ewrite fs1", 0);
	if(ret < 0) {
		printf("write fs1 image fail ...!\n");
		goto out;
	}
...
static int nt72673_a32_tk_eburner_v2(void)
{
	int ret = 0;
