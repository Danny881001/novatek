...
void nt72673_a32_tk_android_oreo_burn_image(void)
{
...
#if defined(CONFIG_ANDROID_AB_UPDATE)
	ret = run_command("ewrite opera0", 0);
#else
	ret = run_command("ewrite opera", 0);
#endif
	if(ret < 0) {
		printf("write opera image fail ...!\n");
		goto out;
	}
	...
	ret = run_command("emark_format_db apdat", 0);
	if(ret < 0) {
		printf("emark_format_db apdat fail ...!\n");
		goto out;
	}
...
static int do_n00010_refresh(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	nvt_vhash_overwrite();
}

U_BOOT_CMD(
		n00010_refresh,3,	0,	do_n00010_refresh,
		"refresh device data\n",
		""
	  );
