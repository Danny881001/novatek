...
static struct ST_KER_VID_PANEL_SETTING gstPanelSetting;
...
static int nvt_eburner_read_panel_param_from_img(int *pindex, panel_param_blk *pparam)
{
	...
	ret = nvt_eburner_load_img(panel_param_tmp_buff, "param", NVT_EBURNER_IMG_FNAME);
	if(ret < 0) {
	...
	ptr += sizeof(int);
	memcpy(pindex, ptr, sizeof(int));
	*pindex = ntohl(*pindex); 

	//read panel param from update image
	ret = nvt_eburner_load_img(panel_param_tmp_buff, "panelparam", NVT_EBURNER_IMG_FNAME);
	if(ret < 0) {
...
static panel_param_blk panel_param_block;
...
void _VID_Get_PanelTablePara( bool b8GetFromTxt )
{
	...
	if( b8GetFromTxt == GET_PANEL_TABLE_FROM_INTERNAL )
	{
		...
		gstPanelSetting.u16PanelPowerGPIO        =           276; 
		gstPanelSetting.u16PanelBackLightGPIO    =           33032;        
	}
