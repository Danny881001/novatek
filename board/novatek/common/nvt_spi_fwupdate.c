/*
 *  board/nvt/evb670btk/nvt_spi_fwupdate.c
 *
 *  Author:	Alvin lin
 *  Created:	Dec 16, 2011
...
#include <nvt_spi_fwupdate.h>

#define NVT_SPI_WRITE_DMA_ALIGN		4

static int nvt_spi_update_bin(SPI_PARTITION partition_name,char*  fname)
{
	char cmd_buf[256] = {0};
	u32 sect_size;
	u32 write_size;
	...
	sprintf(cmd_buf,"fatload usb 0:auto 0x%x %s", CONFIG_SYS_FWUPDATE_BUF,fname);
	...
	remain = (write_size % NVT_SPI_WRITE_DMA_ALIGN);
	
	if(remain) 
		write_size += (NVT_SPI_WRITE_DMA_ALIGN - remain);
	...
	sprintf(cmd_buf,"sf write %x %x %x",CONFIG_SYS_FWUPDATE_BUF,
...
int do_nvt_swrite_cmd(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	int ret = 1;
	int fwup_ret = -1;
	int i;
	...
	if(strcmp(argv[1],"all") == 0) {
		fwup_ret = nvt_spi_update_bin(SPI_STBC, STBC_FNAME);
		
		if(fwup_ret < 0)
			goto err_out;
		
		fwup_ret = nvt_spi_update_bin(SPI_STBCDAT, STBCDAT_FNAME);
		
		if(fwup_ret < 0)
			goto err_out;
		
		fwup_ret = nvt_spi_update_bin(SPI_UBOOT, UBOOT_FNAME);
		
		if(fwup_ret < 0)
			goto err_out;
		
		fwup_ret = nvt_spi_update_bin(SPI_KERNEL, KERNEL_FNAME);
		
		if(fwup_ret < 0)
			goto err_out;
		
		fwup_ret = nvt_spi_update_bin(SPI_FS, FS_FNAME);
		...
		if(argc == 3)
			fwup_ret = nvt_spi_update_bin(SPI_STBC, argv[2]);
		else
			fwup_ret = nvt_spi_update_bin(SPI_STBC, STBC_FNAME);
	} else if(strcmp(argv[1],"stbcdat") == 0) {
		if(argc == 3)
			fwup_ret = nvt_spi_update_bin(SPI_STBCDAT, argv[2]);
		else
			fwup_ret = nvt_spi_update_bin(SPI_STBCDAT, STBCDAT_FNAME);
	} else if(strcmp(argv[1],"uboot") == 0) {
		if(argc == 3)
			fwup_ret = nvt_spi_update_bin(SPI_UBOOT, argv[2]);
		else
			fwup_ret = nvt_spi_update_bin(SPI_UBOOT, UBOOT_FNAME);
	} else if(strcmp(argv[1],"ker")== 0) {
		if(argc == 3)
			fwup_ret = nvt_spi_update_bin(SPI_KERNEL, argv[2]);
		else
			fwup_ret = nvt_spi_update_bin(SPI_KERNEL, KERNEL_FNAME);
	} else if(strcmp(argv[1],"fs") == 0) {
		if(argc == 3)
			fwup_ret = nvt_spi_update_bin(SPI_FS, argv[2]);
		else
			fwup_ret = nvt_spi_update_bin(SPI_FS, FS_FNAME);
	}
		...
		if(argc == 3)
			fwup_ret = nvt_spi_update_bin(SPI_BOARD, argv[2]);
		else
			fwup_ret = nvt_spi_update_bin(SPI_BOARD, BOARD_FNAME);
	}
#endif
	if(fwup_ret < 0)
		goto err_out;
