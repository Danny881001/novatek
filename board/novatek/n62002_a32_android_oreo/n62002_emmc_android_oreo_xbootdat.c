...
#define N62002_3G_ADC_VAL 	0x74
#define N62002_3G_ADC_RANGE 	10

static int n62002_ddr_sz_is_3g(void)
{
	uint32_t reg_val;
	uint32_t val_min = N62002_3G_ADC_VAL - N62002_3G_ADC_RANGE;
	uint32_t val_max = N62002_3G_ADC_VAL + N62002_3G_ADC_RANGE;

	reg_val = __REG(0xFC040324);

	if(reg_val > val_min && reg_val < val_max)
    {
    	printf("MP board, using 3G memory!\n");
		return 1;
	}
    else
    {    
    	printf("Not MP board, using 2G memory!\n");
		return 0;
    }
}
...
static int panel_xdat_dump(struct _nvt_emmc_xbootdat_info *this, int argc, char* const argv[])
{
	...
	//read index
	pimg = nvt_emmc_get_img_by_name("deviceinfo");
	if(pimg == NULL) {
	...
	deviceInfo = (struct HisDeviceInfo *)CONFIG_SYS_FWUPDATE_BUF;
	printf("current panel param index : %d\n", deviceInfo->panel_index);
...
static int panel_xdat_save(struct _nvt_emmc_xbootdat_info *this, int argc, char* const argv[])
{
	...
	if (strcmp(argv[2], "index") == 0) {
		...
		if(idx >= PANEL_PARM_NUM) {
			printf("panel index must between 0~99\n");
			ret = -EINVAL;
			goto out;
		}
...
int nvt_prepare_kercmd(void)
{
	...
	if (getenv("vmalloc") != NULL)
		sprintf(buf,"vmalloc=%s ",getenv("vmalloc"));
	else
		sprintf(buf,"%s ","vmalloc=336M");
