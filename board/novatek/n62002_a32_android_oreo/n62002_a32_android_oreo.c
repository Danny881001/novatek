/*
 *  nt72673_a32_android_oreo.c
 *
 *  Author:	Alvin lin
 *  Created:	Oct 13, 2016
 *  Copyright:	Novatek Inc.
 *
 */
#include <common.h>
#include <dm.h>
#include <errno.h>
#include <malloc.h>
#include <netdev.h>
#include <asm/io.h>
#include <usb.h>
#include <asm/arch/hardware.h>
#include <nvt_emmc_partition.h>
#include <nvt_emmc_fwupdate.h>
#include <nvt_stbc.h>
#include <nvt_emmc_utils.h>
#include <nvt_common_utils.h>
#include <nvt_eburner.h>
#include "hs_type.h"

DECLARE_GLOBAL_DATA_PTR;

int misc_init_r(void)
{
	printf("%s called !\n", __func__);
	return 0;
}

int board_init(void)
{
	gd->bd->bi_arch_number = 0x104d;
	gd->bd->bi_boot_params = 0x06408000;
	return 0;
}

int board_mmc_init(bd_t *bis)
{
	printf("%s called !\n", __func__);
	//nvt_mmc_init();
	return 0;
}

void nt72673_a32_tk_android_oreo_burn_image(void)
{
	int ret;
	u32 reg_val, tmp_val;
	char cmd[128] = {0};

	nvt_setup_emmc_boot_config();

	setenv("usb_port","0");

	run_command("usb start", 0);

	sprintf(cmd, "fatload usb 0:auto 0x%x xboot.bin 4", CONFIG_SYS_FWUPDATE_BUF);

	if(run_command(cmd , 0))
		setenv("usb_port", "3");

	ret = run_command("ewrite xboot", 0);
	if(ret < 0) {
		printf("write xboot fail ...!\n");
		goto out;
	}
	printf("write xboot done ...\n");

#if defined(CONFIG_NVT_TK_SPI_UTILS)
	ret = run_command("swrite stbc stbc.bin", 0);

	if(ret < 0) {
		printf("write spi stbc fail ...!\n");
		goto out;
	}
	printf("write spi stbc done ...\n");
#endif

	ret = run_command("ewrite stbc stbc_emmc.bin", 0);
	if(ret < 0) {
		printf("write stbc_emmc fail !\n");
		goto out;
	}

	ret = run_command("ewrite ddrcfg", 0);

	if(ret < 0) {
		printf("write ddrcfg fail !\n");
		goto out;
	}

	ret = run_command("ewrite uboot", 0);
	if(ret < 0) {
		printf("write U-Boot fail ...!\n");
		goto out;
	}
	printf("write U-Boot done...\n");

	ret = run_command("ewrite edid", 0);
	if(ret < 0) {
		printf("write edid fail ...!\n");
		goto out;
	}
	printf("write edid done...\n");

	ret = run_command("xsave panel config xbootdat.bin", 0);
	if(ret < 0) {
		printf("write panel config fail !\n");
		goto out;
	}

	ret = run_command("ewrite fdt0", 0);
	if(ret < 0) {
		printf("write fdt0 fail ...!\n");
		goto out;
	}
	printf("write fdt0 done...\n");

	ret = run_command("ewrite fdt1", 0);
	if(ret < 0) {
		printf("write fdt1 fail ...!\n");
		goto out;
	}
	printf("write fdt1 done...\n");

	ret = run_command("ewrite ker0", 0);
	if(ret < 0) {
		printf("write kernel fail ...!\n");
		goto out;
	}
	printf("write kernel done...\n");

	ret = run_command("ewrite ker1", 0);
	if(ret < 0) {
		printf("write recovery fail ...!\n");
		goto out;
	}
	printf("write recovery done...\n");

	ret = run_command("ewrite kerupm", 0);
	if(ret < 0) {
		printf("write kerupm fail ...!\n");
		goto out;
	}
	printf("write kerupm done...\n");

	ret = run_command("epart write", 0);
	if(ret < 0) {
		printf("write partition fail ...!\n");
		goto out;
	}
	printf("write partition done ...\n");

#if defined(CONFIG_ANDROID_AB_UPDATE)
	ret = run_command("ewrite system0", 0);
#else
	ret = run_command("ewrite system", 0);
#endif
	if(ret < 0) {
		printf("write system image fail ...!\n");
		goto out;
	}

	ret = run_command("ewrite cache", 0);
	if(ret < 0) {
		printf("write cache image fail ...!\n");
		goto out;
	}

#if defined(CONFIG_ANDROID_AB_UPDATE)
	ret = run_command("ewrite vendor0", 0);
#else
	ret = run_command("ewrite vendor", 0);
#endif
	if(ret < 0) {
		printf("write vendor image fail ...!\n");
		goto out;
	}

	ret = run_command("ewrite tvos", 0);
	if(ret < 0) {
		printf("write tvos image fail ...!\n");
		goto out;
	}

	ret = run_command("ewrite apdat", 0);
	if(ret < 0) {
		printf("write apdat image fail ...!\n");
		goto out;
	}

	ret = run_command("ewrite tvconfig", 0);
	if(ret < 0) {
		printf("write tvconfig image fail ...!\n");
		goto out;
	}

	ret = run_command("ewrite data", 0);
	if(ret < 0) {
		printf("write data image fail ...!\n");
		goto out;
	}

	run_command("emark_format_db tvdatabase", 0);
	run_command("emark_format_db logofs", 0);
	run_command("emark_format_db hsdtv", 0);

	run_command("emiscmd \"\"", 0);

	ret = run_command("xsave kercmd", 0);
	if(ret < 0) {
		printf("generate kernel command line fail !\n");
		goto out;
	}

	ret = run_command("xsave rcvcmd", 0);
	if(ret < 0) {
		printf("generate recovery command line fail !\n");
		goto out;
	}

	printf("Burning image process complete, please reboot this device.\n");

	//auto reboot
	nvt_stbc_set_keypass(1);

	//switch authority
	__REG(0xFC04041C) |= ((1 << 2) | (1 << 0));

	//set STBC_GPIO0,2 set as input
	__REG(0xFC040438) |= (1 << 6);
	__REG(0xFC04042C) &= ~(1 << 6);
	__REG(0xFC040438) |= (1 << 8);
	__REG(0xFC04042C) &= ~(1 << 8);

	__REG(0xFC040210) = 0xF0;

	while(1);

out:
	return;
}

static int do_nvt_ewriteall(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	nt72673_a32_tk_android_oreo_burn_image();
}

U_BOOT_CMD(
		ewriteall,3,	0,	do_nvt_ewriteall,
		"ewriteall    - write all images to emmc\n",
		""
	  );


#if defined(CONFIG_NVTTK_EBURNER_V2)
/*
 * Anti-board type unmatch mechanism:
 * Only board type had written on board and board type simutaneously in the upgrade image should check.  
 * Other situation need upgrade directly.
 */
#define DEVICE_INFO_PARTITION	"deviceinfo"
#define DEVICE_INFO_IMAGE		"board_info"
static int do_board_info_check()
{
	int ret = 0;
	char cmd[128] = {};
    struct HisDeviceInfo deviceInfo = {0};

	sprintf(cmd, "eread 0x%x %s", CONFIG_SYS_FWUPDATE_BUF, DEVICE_INFO_PARTITION);
	if(run_command(cmd , 0))
	{	
		/* Read partition failed, need reboot */
		printf("Read deviceinfo partition failed!\n");
        return -1;
	}

    memcpy(&deviceInfo, (char*)CONFIG_SYS_FWUPDATE_BUF, sizeof(struct HisDeviceInfo));

    ret = nvt_eburner_load_img((void*)CONFIG_SYS_FWUPDATE_BUF, DEVICE_INFO_IMAGE, NVT_EBURNER_IMG_FNAME);
    if(ret)
    {
        printf("No board type setting in upagrade image, need upgrade!\n");
        return 0;
    }

	if(strlen(deviceInfo.local_his_string) == 0)
    {
        printf("No board type setting on board, need upgrade!\n");
        return 0;
    }
    
    if(memcmp(deviceInfo.local_his_string, (char*)CONFIG_SYS_FWUPDATE_BUF, 16))
    {
    	printf("Board tyep not match!\n");
        return -1;
    }
    else
   	{
   		printf("Board type match!\n");
        return 0;
   	}     
}


static int n62002_eburner_v2(void)
{
	int ret = 0;
	char cmd[128] = {};

	printf("hs>>in-upgrade\r\n");
    
	nvt_setup_emmc_boot_config();
	run_command("epart write", 0);
	run_command("emiscmd \"\"", 0);
	
	run_command("usb reset", 0);

	sprintf(cmd, "fatload usb 0:auto 0x%x %s 4", CONFIG_SYS_FWUPDATE_BUF, NVT_EBURNER_IMG_FNAME);

	//if update image not exist, boot to recovery mode
	if(run_command(cmd , 0)) {
		printf("update file %s not exist, try classic empty burn mode \n", NVT_EBURNER_IMG_FNAME);
		run_command("ewriteall", 0);
	#if 0
		printf("update file %s not exist, boot to recovery mode \n", NVT_EBURNER_IMG_FNAME);
		run_command("emiscmd boot-recovery", 0);
	#endif
		run_command("reboot", 0);
	}

    //Do board info check
    if(do_board_info_check())
	{
		printf("Upgrade file %s is not suitable for this board, reboot now!\n", NVT_EBURNER_IMG_FNAME);
		//TODO : temporarily marked this function.
		//run_command("reboot", 0);
	}

	setenv("quiet", "y");
    setenv("cpus", "1");

	ret = nvt_eburner_run_updater();

	if(ret < 0) {
		printf("%s run empty burner v2 fail !\n", __func__);
		while(1);
	}

	return ret;
}

static int do_nvt_eburner(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	n62002_eburner_v2();
}

U_BOOT_CMD(
		nvt_eburner,3,	0,	do_nvt_eburner,
		"nvt_eburner    - run novatek eburner to update image\n",
		""
	  );


#endif//CONFIG_NVTTK_EBURNER_V2

#if defined(CONFIG_NVT_EUPM)

static int do_nvt_hisense_raw_dump(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	setenv("eupm_param", "hisense_raw_dump");
	run_command("eupm", 0);
}
U_BOOT_CMD(
		hisense_raw_dump,   1,  0,  do_nvt_hisense_raw_dump,
		"hisense_raw_dump",
		""
	  );

static int do_nvt_hisense_raw_restore(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	setenv("eupm_param", "hisense_raw_restore");
	run_command("eupm", 0);
}
U_BOOT_CMD(
		hisense_raw_restore,    1,  0,  do_nvt_hisense_raw_restore,
		"hisense_raw_restore",
		""
	  );

#endif //CONFIG_NVT_EUPM

int board_late_init(void)
{
	nvt_stbc_disable_wdog();

#if defined(CONFIG_NVT_TK_EMMC_UTILS)
	nvt_emmc_part_init();
#endif

#if defined(CONFIG_NVTTK_NO_EBURNER)
	return 0;
#endif
	if(nvt_in_empty_burner_mode()||\
        nvt_stbc_read_power_on_event()==\
        EN_DRV_STBC_POWER_ON_EVENT_UPGRADE)
    {
	#if defined(CONFIG_NVTTK_EBURNER_V2)
		run_command("nvt_eburner", 0);
	#else
		run_command("ewriteall", 0);
	#endif
	}
	return 0;
}
