...
#define PANLE_PARAM_PARTITION   "panelparam"
...
static int nvt_eburner_read_panel_param_from_img(int *pindex, panel_param_blk *pparam)
{
    ...
    struct HisDeviceInfo* deviceInfo;

	//read index from update image
	ret = nvt_eburner_load_img(panel_param_tmp_buff, DEVICE_INFO_PARTITION, NVT_EBURNER_IMG_FNAME);
	if(ret < 0) {
		printf("%s read panel index from image fail !\n", __func__);
		ret = -EINVAL;
		*pindex = -1;
		goto out;
	}

	deviceInfo = (struct HisDeviceInfo *)panel_param_tmp_buff;
	*pindex = deviceInfo->panel_index; 

    printf("panel index: %d\n", *pindex);

	//read panel param from update image
	ret = nvt_eburner_load_img(panel_param_tmp_buff, PANLE_PARAM_PARTITION, NVT_EBURNER_IMG_FNAME);
	if(ret < 0) {
		printf("%s read panel param from image fail !\n", __func__);
		ret = -EINVAL;
		*pindex = -1;
		goto out;
	}
...
static int nvt_eburner_read_panel_param_from_dev(int *pindex, panel_param_blk *pparam)
{
	...
	pimg = nvt_emmc_get_img_by_name(DEVICE_INFO_PARTITION);
	if(pimg == NULL) {
	...
	*pindex = deviceInfo->panel_index; 
	...
	//read panel param from dev partition
	pimg = nvt_emmc_get_img_by_name("xbootdat");
	if(pimg == NULL) {
		printf("%s read panelparam partition fail !\n", __func__);
		ret = -EINVAL;
		*pindex = -1;
		goto out;
	}
