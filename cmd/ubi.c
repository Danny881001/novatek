...
static int get_user_volume_num(struct ubi_device *ubi)
{
	int i;
	int num = 0;
	...
	for (i = 0; i < (ubi->vtbl_slots + 1); i++) {
		if (!ubi->volumes[i])
			continue;	/* Empty record */
		if (!strcmp(ubi->volumes[i]->name, "nvt_sign"))
			continue;
		if (!strcmp(ubi->volumes[i]->name, "layout volume"))
			continue;
		num ++;
	}
	return num;
...
static int check_volume_info(struct ubi_device *ubi, const char *vol_name, int64_t vol_size, int vol_id)
{
	int i;
	int64_t check_vol_size;

	for (i = 0; i < (ubi->vtbl_slots + 1); i++) {
		if (!ubi->volumes[i])
			continue;	/* Empty record */
		if (!strcmp(ubi->volumes[i]->name, vol_name)
			&& strlen(ubi->volumes[i]->name)== strlen(vol_name)) {
			/* Check size */
			check_vol_size = ubi->volumes[i]->reserved_pebs * ubi->volumes[i]->usable_leb_size;
			if (check_vol_size == vol_size) {
				/* Check id */
				if (vol_id == ubi->volumes[i]->vol_id)
					return 0;
			}
		}
	}
...
static int ubi_create_vol_with_id(char *volume, int64_t size, int dynamic, int vol_id)
{
	struct ubi_mkvol_req req;
	int err;
...
static int do_ubi(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (strncmp(argv[1], "nvt_create", strlen("nvt_create")) == 0 &&
		strlen(argv[1]) == strlen("nvt_create")) {
		int dynamic = 1;	/* default: dynamic volume */
		int vol_id = -1;
		int ret = 0;
		...
		if (argc == 6) {
			vol_id = simple_strtol(argv[5], NULL, 10);
			argc --;
		} else {
			printf("Invalid arg number, Please see usage\n");
			return 1;
		}
		...
		/* E.g., create volume */
		if (argc == 3)
			ret = ubi_create_vol_with_id(argv[2], size, dynamic, vol_id);
		return ret == 0 ? 0:1;
	}
	...
	if (strncmp(argv[1], "nvt_checkvolnum", strlen("nvt_checkvolnum")) == 0 &&
		strlen(argv[1]) == strlen("nvt_checkvolnum")) {
		int num = 0;
		...
		if (num != get_user_volume_num(ubi)) {
			/* FAIL */
			return 1;
		} else {
			/* PASS */
	...
	if (strncmp(argv[1], "nvt_checkvol", strlen("nvt_checkvol")) == 0 &&
		strlen(argv[1]) == strlen("nvt_checkvol")) {
		int id = -1;
		...
		size = simple_strtoull(argv[3], NULL, 16);
		id = simple_strtol(argv[4], NULL, 10);
		return check_volume_info(ubi, argv[2], size, id);
	}

	if (strncmp(argv[1], "nvt_removeall", strlen("nvt_removeall")) == 0 &&
		strlen(argv[1]) == strlen("nvt_removeall")) {
		int i;
		int ret = 0;
...
U_BOOT_CMD(
	...
	"ubi nvt_create[vol] volume [size] [type] [vol_id]"
		" - create volume name with size and id\n"
	...
	"ubi remove[vol] volume"
		" - Remove volume\n"
	"ubi nvt_checkvolnum volume_num"
		" - Check number of user volume match or not\n"
	"ubi nvt_checkvol volume size id"
		" - Check volume info match or not\n"
	"ubi nvt_removeall"
		" - Remove all volume\n"
	"[Legends]\n"
