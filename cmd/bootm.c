...
typedef void (*LDR_GENERIC_CB)(int, int);
#define JUMP_ADDR 0xf07f0000

int do_nvt_ddr_scan(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	int dram1_size=0, dram2_size=0;
	char *endp;
	void (*image_entry)(int, int) = NULL;

	if (argc >= 2) {
		dram1_size = simple_strtoul(argv[1], &endp, 10);
	}
	if (argc >= 3) {
		dram2_size = simple_strtoul(argv[2], &endp, 10);
	}
	...
	printf("ARG: %d %d\r\n", dram1_size, dram2_size);
	image_entry = (LDR_GENERIC_CB)(*((unsigned long*)JUMP_ADDR));
	image_entry(dram1_size, dram2_size);

	return 0;
}

U_BOOT_CMD(
	nvt_ddr_scan, 3,	1,	do_nvt_ddr_scan,
	"nvt ddr scan",
	"<dram1 size> <dram2 size>\r\n"
	"dram1 size - \t0/128/256/512/1024unit: MBytes\r\n"
	"dram2 size - \t0/64/128/256/512 (unit: MBytes)\r\n"
);
