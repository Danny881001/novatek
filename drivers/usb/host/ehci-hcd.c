...
static int ehci_shutdown(struct ehci_ctrl *ctrl)
{
	...
	if (!ret) {
		for (i = 0; i < CONFIG_SYS_USB_EHCI_MAX_ROOT_PORTS; i++) {
			...
			/*
			 The behavior that uboot makes USB bus into suspend
			 causes linux kernel can't handle USB controller well.
			 So we just disable port power state.
			 Or linux kernel should trigger USB controller reset bit
			 to reocver suspend state
			 */
			reg &= ~EHCI_PS_PE;
//			reg |= EHCI_PS_SUSP;
			...
			void *buffer, int interval)
{
	...
	if (elementsize > usb_maxpacket(dev, pipe)) {
		printf("%s: xfers requiring several transactions are not supported.\n",
		       __func__);
#ifndef CONFIG_NVT_RTK_BT_FWLOAD
		return NULL;
#endif
	}
