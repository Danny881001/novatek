...
static int nvt_emmc_arch_host_preinit(struct mmc_nvt_host *host)
{
	if (host->id == SDIO_HOST_ID_1) {
		...
		*(uint32_t*) 0xF0010004 &= ~(0x81003000);
		udelay(10);
		if (host->pinmux_value & PIN_SDIO_CFG_2ND_PINMUX) {
			if (host->enable_8bits) {
				*(uint32_t*) 0xF0010004 |= 0x81020000;
				udelay(10);
				*(uint32_t*) 0xF00100A0 &= ~(0xEFF);
			} else {
				*(uint32_t*) 0xF0010004 |= 0x1020000;
				udelay(10);
				*(uint32_t*) 0xF00100A0 &= ~(0xE0F);
			}
		} else {
			*(uint32_t*) 0xF0010004 |= 0x20000;
			udelay(10);
			*(uint32_t*) 0xF00100A0 &= ~(0xFC000000);
		}
