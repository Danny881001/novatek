...
/*
 * erase/write start to unprotect all SPI
 *
 * erase/write end to protect all SPI
 */

int write_protect(struct spi_flash *flash)
{
	u8 status = 0;
	u8 cmd[2];
	int ret;
...
int write_unprotect(struct spi_flash *flash, u32 ofs, size_t len)
{
	u8 status = 0;
	u8 cmd[2];
	int ret;
