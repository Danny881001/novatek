...
{
#ifndef CONFIG_DISABLE_SPI_LOCK_CTRL
...
static void spi_access_start(spi_regs_t *spi_reg)
{
	...
	val = spi_reg->ctrl;
	spi_reg->ctrl= (val | SPI_DEFAULT_VALUE);
	...
	do
	{
		...
		{
#ifndef CONFIG_DISABLE_SPI_LOCK_CTRL
			spi_reg->ctrl 			= (SPI_CTRL_LOCK | SPI_DEFAULT_VALUE);	/* keep bus lock state */
#else
            spi_reg->ctrl           = SPI_DEFAULT_VALUE;
#endif
			spi_reg->cmd			= 0;
			...
			spi_out32(SPI_CTRL_REG, spi_reg->ctrl);
			//printf("SPI_CTRL_REG=%x\n",*((volatile unsigned int *)SPI_CTRL_REG));
			spi_out32(SPI_CMD_REG, spi_reg->cmd);
			spi_out32(SPI_ACCESS_CTRL_REG, spi_reg->access_ctrl);
			break;
		}
...
static int spi_data_ahbdma_read(spi_regs_t *spi_reg, unsigned int *buf, unsigned int len)
{
	...
	invalidate_dcache_range((ulong)buf, ((ulong)(buf+len+CONFIG_SYS_CACHELINE_SIZE) & ~ (CONFIG_SYS_CACHELINE_SIZE-1)));
...
static int spi_data_ahbdma_write(spi_regs_t *spi_reg, unsigned int *buf, unsigned int len)
{
	...
	/* note: it needs flush the cache when a dma operation */
	flush_dcache_range((ulong)buf, ((ulong)(buf+len+CONFIG_SYS_CACHELINE_SIZE) & ~ (CONFIG_SYS_CACHELINE_SIZE-1)));
			...
			unsigned int max_hz, unsigned int mode)
{
	...
	memset(slave, 0 , sizeof(struct spi_slave));
		...
		const void *dout, void *din, unsigned long flags)
{
	...
	#if defined(CONFIG_SPI_APB_DMA_MODE)
	spi_init_apbdma_tzasc();
	#endif
	#if defined(CONFIG_SPI_AHB_DMA_MODE)
	...
	else if (spi_params.dir == XFER_DIR_TO_SPI)
	{
		if (spi_params.command == CMD_SPI_WRSR)
		{
			...
			{
				//for winbond only
				spi_out8((SPI_DATA0_REG + 0), ((unsigned char *)txd)[0]);
				spi_out8((SPI_DATA0_REG + 1), ((unsigned char *)txd)[1]);
			}
			else
			{
...
void spi_flash_wp(int spi_protect)
{
    if(spi_protect > 0)
    {
        //Protect
        //Low level
        __REG(0xfc040420) &= (~(0x1<<18));
        //output mode
        __REG(0xfc04042c) |= (0x1<<18);
    }
    else
    {
        //unprotect
