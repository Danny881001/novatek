...
int nand_get_bad_number(struct mtd_info *mtd, loff_t offset, size_t length)
{
	size_t bad_num = 0;
	size_t block_start;
	while (length > 0) {
...
int nand_write_skip_bad_oob(struct mtd_info *mtd, size_t offset, size_t *length,
                      u_char *buffer,size_t *oob_len,u_char *oob_buf, int flags)
{
	int rval = 0, blocksize;
	size_t left_to_write = *length;
	size_t used_for_write = 0;
	...
	if (!need_skip && !(flags & WITH_DROP_FFS)) {
		...
		pages = DIV_ROUND_UP(left_to_write, pagesize);
		for(page=0;page < pages;page++) {
			ops.datbuf = buffer + (page * pagesize);
	...
	while (left_to_write > 0) {
		...
		pages = DIV_ROUND_UP(truncated_write_size, pagesize);
		for(page=0;page < pages;page++) {
			ops.datbuf = buffer + (page * pagesize);
			rval = mtd->_write_oob(mtd, offset, &ops);
			if(rval != 0) {
