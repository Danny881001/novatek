		...
		if (info->flash_info->plane_flags && (uiRowAddr & (0x1 << 6))) {
			...
			*(u32*) 0xF00200E0 &= ~0x80000;
			...
			if (uiColAddr == info->flash_info->page_size) {
				...
				flush_dcache_range((UINT32)pulBuf, (UINT32)(pulBuf + info->flash_info->oob_size/4));
			} else {
				nand_host_setup_transfer(info, &transParam, 1, info->flash_info->page_size + info->flash_info->oob_size, _SINGLE_PAGE);

				flush_dcache_range((UINT32)pulBuf, (UINT32)(pulBuf + (info->flash_info->page_size + info->flash_info->oob_size)/4));
			}
			...
			if (uiColAddr == info->flash_info->page_size)
				invalidate_dcache_range((UINT32)pulBuf, (UINT32)(pulBuf + info->flash_info->oob_size/4));
			else
				invalidate_dcache_range((UINT32)pulBuf, (UINT32)(pulBuf + (info->flash_info->page_size + info->flash_info->oob_size)/4));
			...
			*(u32*) 0xF00200E0 |= 0x80000;
			...
			if(uiColAddr == info->flash_info->page_size) {
				...
				//#NT#Modify parameter for multispare read flow
		...
		if (uiColAddr & 0xFFF) {
			...
			*(u32*) 0xF00200E0 |= 0x80000;
			...
			if (uiBufSize > info->flash_info->page_size) {
				flush_dcache_range((UINT32)pulBuf, (UINT32)(pulBuf + info->flash_info->page_size/4));
				nand_host_setup_transfer(info, &transParam, 1, info->flash_info->page_size, _SINGLE_PAGE);
			} else {
				flush_dcache_range((UINT32)pulBuf, (UINT32)(pulBuf + uiBufSize/4));
...
void nand_cmd_write_spare_sram(struct drv_nand_dev_info *info)
{
	...
	flush_dcache_range((UINT32)pul_buf, (UINT32)(pul_buf + info->flash_info->oob_size/4));
